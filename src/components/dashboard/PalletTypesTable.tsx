...
const getTableName = (inventoryType: string) => {
  if (inventoryType === "ambalaje") return "ambalaje_pallet_types";
  if (inventoryType === "etichete") return "etichete_pallet_types";
  return "pallet_types";
};

const PalletTypesTable: React.FC = () => {
  const { inventoryType } = useInventoryType();
  const [items, setItems] = useState<PalletType[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editItem, setEditItem] = useState<PalletType | null>(null);
  const [isAddingNew, setIsAddingNew] = useState(false);
  const [newItem, setNewItem] = useState<Partial<PalletType>>({ name: "", description: "" });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setItems((data as PalletType[]) || []);
    } catch (error: any) {
      toast({ variant: "destructive", title: "Eroare la încărcare tip paleți", description: error.message });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!window.confirm(`Sigur ștergi tipul de palet "${name}"?`)) return;
    try {
      const { error } = await (supabase as any)
        .from(getTableName(inventoryType))
        .delete()
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Tipuri de paleți</h2>
        <Button onClick={() => setIsAddingNew(true)} disabled={isAddingNew}>
          <Plus className="h-4 w-4 mr-2" /> Adaugă tip palet
        </Button>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          {isAddingNew && (
            <TableRow>
              <TableCell>
                <Input value={newItem.name || ""} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} placeholder="ex: Palet lemn alb" />
              </TableCell>
              <TableCell>
                <Input value={newItem.description || ""} onChange={(e) => setNewItem({ ...newItem, description: e.target.value })} placeholder="Descriere" />
              </TableCell>
              <TableCell>
          ...
          {items.length === 0 && !isAddingNew && !loading && (
            <TableRow>
              <TableCell colSpan={3} className="text-center py-8 text-muted-foreground">
                Nu există tipuri de paleți. Adaugă unul nou.
              </TableCell>
            </TableRow>
          )}
          {loading && (
...
export default PalletTypesTable;
