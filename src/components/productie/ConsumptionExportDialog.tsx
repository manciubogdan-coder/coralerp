...
const COLUMN_DEFS = [
  { key: "nume", label: "Ingredient" },
  { key: "cantitate_necesara", label: "Cantitate Necesară (kg)" },
  { key: "cantitate_consumata", label: "Cantitate Consumată (kg)" }
];
...
const ConsumptionExportDialog: React.FC<Props> = ({ consumptionData, fileName }) => {
  ...
  const exportData = () => {
    const dataToExport = consumptionData.map(item => {
      return selectedCols.reduce((acc, key) => {
        let value = item[key];
        
        // Formatăm valorile în kg cu 2 zecimale
        if (key === "cantitate_necesara" || key === "cantitate_consumata") {
          value = typeof value === "number" ? value.toFixed(3) : value || "0.000";
        }
    ...
    if (exportFormat === "csv") {
      ...
      const ws_data = [header].concat(
        dataToExport.map(row => 
          COLUMN_DEFS.filter(col => selectedCols.includes(col.key)).map(col => row[col.key])
        )
      );
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Export Consumuri Materii Prime</DialogTitle>
          <DialogDescription>
...
export default ConsumptionExportDialog;
