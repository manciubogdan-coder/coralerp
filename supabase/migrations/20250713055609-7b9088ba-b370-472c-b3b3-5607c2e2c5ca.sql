-- Actualez trigger-ul pentru a calcula net_quantity și la UPDATE, dar fără proporționalitate
...
BEGIN
  ...
  -- La UPDATE, recalculez net_quantity bazat pe noua cantitate brută și datele originale
  IF TG_OP = 'UPDATE' THEN
    ...
    ELSE
      -- Păstrez valorile originale de lăzi și greutate din recepție
      -- și calculez net_quantity bazat pe noua cantitate brută
      IF NEW.crate_type_id IS NOT NULL AND NEW.crate_count > 0 THEN
        SELECT NEW.quantity - (ct.weight * NEW.crate_count) - COALESCE(NEW.crate_weight, 0)
        INTO NEW.net_quantity
        FROM public.crate_types ct 
...
-- Aceeași logică pentru ambalaje
...
BEGIN
  ...
  -- La UPDATE, recalculez bazat pe cantitatea nouă dar cu valorile originale de lăzi
  IF TG_OP = 'UPDATE' THEN
    IF NEW.quantity = 0 THEN
      NEW.crate_count := 0;
      NEW.crate_weight := 0;
      ...
      NEW.gross_quantity := 0;
    ELSE
      -- Calculez net_quantity bazat pe noua cantitate brută și lăzile originale
      IF NEW.crate_type_id IS NOT NULL AND NEW.crate_count > 0 THEN
        SELECT NEW.quantity - (ct.weight * NEW.crate_count) - COALESCE(NEW.crate_weight, 0)
        INTO NEW.net_quantity
        FROM public.ambalaje_crate_types ct 
