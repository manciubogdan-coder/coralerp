-- Problema: transferul modifică cantitatea din recepția originală prin trigger
-- Soluția: separăm logica de recepție de cea de consum/transfer

-- 1. Dezactivez trigger-ul care recalculează proporțional
DROP TRIGGER IF EXISTS calculate_quantities_trigger ON inventory;
DROP TRIGGER IF EXISTS calculate_quantities_ambalaje_trigger ON ambalaje_inventory;

-- 2. Creez trigger nou care să NU modifice proporțional la UPDATE
CREATE OR REPLACE FUNCTION public.calculate_quantities_reception_only()
RETURNS TRIGGER AS $$
BEGIN
  -- La INSERT (recepție nouă), calculez cantitățile
  IF TG_OP = 'INSERT' THEN
    -- Setez gross_quantity = quantity la recepție
    NEW.gross_quantity := NEW.quantity;
    
    -- Calculez net_quantity scăzând greutatea lăzilor și paletului
    IF NEW.crate_type_id IS NOT NULL AND NEW.crate_count > 0 THEN
      SELECT NEW.quantity - (ct.weight * NEW.crate_count) - COALESCE(NEW.crate_weight, 0)
      INTO NEW.net_quantity
      FROM public.crate_types ct 
      WHERE ct.id = NEW.crate_type_id;
      
      IF NEW.net_quantity IS NULL THEN
        NEW.net_quantity := NEW.quantity - COALESCE(NEW.crate_weight, 0);
      END IF;
    ELSE
      NEW.net_quantity := NEW.quantity - COALESCE(NEW.crate_weight, 0);
    END IF;
    
    -- Asigur că net_quantity nu devine negativă
    NEW.net_quantity := GREATEST(NEW.net_quantity, 0);
  END IF;
  
  -- La UPDATE, DOAR resetez valorile la 0 dacă quantity = 0
  -- NU mai recalculez proporțional - aceasta era problema!
  IF TG_OP = 'UPDATE' THEN
    IF NEW.quantity = 0 THEN
      NEW.crate_count := 0;
      NEW.crate_weight := 0;
      NEW.net_quantity := 0;
      NEW.gross_quantity := 0;
    END IF;
  END IF;

  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger pentru inventory principal
CREATE TRIGGER calculate_quantities_reception_only_trigger
  BEFORE INSERT OR UPDATE ON inventory
  FOR EACH ROW
  EXECUTE FUNCTION calculate_quantities_reception_only();

-- 3. Aceeași funcție pentru ambalaje
CREATE OR REPLACE FUNCTION public.calculate_quantities_ambalaje_reception_only()
RETURNS TRIGGER AS $$
BEGIN
  -- La INSERT (recepție nouă), calculez cantitățile
  IF TG_OP = 'INSERT' THEN
    NEW.gross_quantity := NEW.quantity;
    
    -- Calculez net_quantity pentru ambalaje
    IF NEW.crate_type_id IS NOT NULL AND NEW.crate_count > 0 THEN
      SELECT NEW.quantity - (ct.weight * NEW.crate_count) - COALESCE(NEW.crate_weight, 0)
      INTO NEW.net_quantity
      FROM public.ambalaje_crate_types ct 
      WHERE ct.id = NEW.crate_type_id;
      
      IF NEW.net_quantity IS NULL THEN
        NEW.net_quantity := NEW.quantity - COALESCE(NEW.crate_weight, 0);
      END IF;
    ELSE
      NEW.net_quantity := NEW.quantity - COALESCE(NEW.crate_weight, 0);
    END IF;
    
    NEW.net_quantity := GREATEST(NEW.net_quantity, 0);
  END IF;
  
  -- La UPDATE, DOAR resetez la 0 dacă quantity = 0
  IF TG_OP = 'UPDATE' THEN
    IF NEW.quantity = 0 THEN
      NEW.crate_count := 0;
      NEW.crate_weight := 0;
      NEW.net_quantity := 0;
      NEW.gross_quantity := 0;
    END IF;
  END IF;

  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger pentru ambalaje_inventory
CREATE TRIGGER calculate_quantities_ambalaje_reception_only_trigger
  BEFORE INSERT OR UPDATE ON ambalaje_inventory
  FOR EACH ROW
  EXECUTE FUNCTION calculate_quantities_ambalaje_reception_only();